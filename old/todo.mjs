...
function saveToStorage() {
  const listsToStore = [];
  lists.forEach((list) => { 
    if (list.name === activeList) { // Ensure to update the active list with any changes
      list.items = items;
    }
    listsToStore.push(list); 
  });
  let state = {
  ...
  try {
    ...
    console.info("Todo lists have been saved with active-list: " + activeList);
  }
  catch (error) {
...
function activateList(listName, updateListOptions) {
  ...
  let selectOrCreateListEl = document.getElementById("select-list");
...
function onDomReady() {
  ...
  let selectListItemEl = document.getElementById("select-list");
  selectListItemEl.addEventListener('change', selectList);
...
function selectList(event) {
  const selectListEl = document.getElementById("select-or-create-list");
  const selectedOption = selectListEl.options[selectListEl.selectedIndex].value;
